...
func SessionHandler(c *gin.Context) {
	...
	if err == nil {
		sessionString, err := redis.GetRedisClient().Get(context.TODO(), cookie).Result()
	...
	redis.GetRedisClient().Set(c, sessionKey, jsonString, time.Duration(expireTime)*time.Second)
}
...
func (s *Session) Set(key string, val interface{}) error {
	...
	redis.GetRedisClient().Set(context.TODO(), s.Key, sessionStringNew, time.Duration(e)*time.Second)
	return nil
}
